...
namespace Euclid.Analytics.Regressions
{
    ...
    public class LazyPartialLeastSquaresLinearRegression<T, TV> where T : IEquatable<T>, IComparable<T> where TV : IEquatable<TV>, IConvertible
    {
        #region Declarations
        ...
        public LazyPartialLeastSquaresLinearRegression(DataFrame<T, double, TV> x, Series<T, double, TV> y)
        {
        ...
        public void Regress()
        {
            ...
            if (intm == null)
            {
                ...
                {
                    _linearModel = LinearModel.Create(yb, yb, n, sst);
            ...
            _linearModel = LinearModel.Create(yb, beta0, beta.ToArray(), correls, n, sse, sst - sse);
            _status = RegressionStatus.Normal;
        }
